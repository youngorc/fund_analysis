...
def get_fund_code_data(file,fund_key,fund_code):
    try:
        df_file=pd.read_excel(file,sheet_name=fund_key,dtype={"代码":str})
    except xlrd.biffh.XLRDError:
...
def data_analysis(fund,df):
    ...
    df=df[['净值日期','单位净值','累计净值','代码','最低价','最高价','D值','J值','K值','RSV','dea','diff','ema12','ema26'\
        ,'macd','boll_mid','boll_std','boll_top','boll_bottom']]
...
def multithread_crawl_data(config):
    ...
    with ThreadPoolExecutor(max_workers=2) as executor_multhread:
        crawl_task={}
        data_tmp={}
