...
def get_fund_code_value(fund):
    ...
    return date,value_danwei,value_gusuan
...
def plot_graph(df,fund,last_days=70):
...
def fund_data_analysis(funds,edate=(datetime.datetime.today() - datetime.timedelta(days=1)).strftime("%Y-%m-%d")):
    for fund in funds:
        ...
        date,value_danwei,value_gusuan=get_fund_code_value(fund)
        if value_gusuan != None:
